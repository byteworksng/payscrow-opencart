...
<<<<<<< HEAD
        ...
        $productModel = $this->model_catalog_product;


        array_walk($products, function(&$product) use ($productModel, $order_info){

            $item = $productModel->getProduct($product['product_id']);


            $product['description'] = $item['description'];

            $product['image'] = $item['image'];
            $product['tax'] = $this->currency->format($product['tax'], $order_info['currency_code'], $order_info['currency_value'], false);
             $product['price'] = $this->currency->format($product['price'], $order_info['currency_code'], $order_info['currency_value'], false);
...
<<<<<<< HEAD
            ...
            $this->model_extension_payment_payscrow->logger('calling live server');
        } else {
            $gatewayUrl = "http://ps.ofekpc.com/api/paymentconfirmation?transactionId={$params['transactionId']}";
            $this->model_extension_payment_payscrow->logger('calling demo server');
        }

                $result =  $this->verifyRequest($gatewayUrl);
                  $this->model_extension_payment_payscrow->logger('result received from validation');
                    $this->model_extension_payment_payscrow->logger($result);
